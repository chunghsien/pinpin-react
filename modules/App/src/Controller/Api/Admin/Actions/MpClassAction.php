...
class MpClassAction extends AbstractAction
{
    ...
    protected function get(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $response = $ajaxFormService->getProcess($request, new MpClassTableGateway($this->adapter));
        if($response->getStatusCode() == 200) {
            ...
            return $apiQueryService->processPaginator(
                $request,
                'modules/App/scripts/db/admin/mpClass.php',
                //欄位對應的資料表名稱
                [
                    'name' => 'mp_class',
                    'sort' => 'mp_class',
    ...
    {
        $ajaxFormService = new AjaxFormService();
        return $ajaxFormService->deleteProcess($request, new MpClassTableGateway($this->adapter));
    }
    ...
    {
        $ajaxFormService = new AjaxFormService();
        $tablegateway = new MpClassTableGateway($this->adapter);
