...
const ProductsSpecAttrsForm = (props) => {
  ...
  const products_spec_attrs = formRows ? formRows.products_spec_attrs : null;
  ...
  useEffect(() => {
    ...
    if (products_spec_attrs) {
      setFormSelected({
        language_has_locale: products_spec_attrs.language_has_locale,
      });
    }

  }, [products_spec_attrs, count]);
      ...
      <CTabContent>
        <CTabPane data-tab="default-form">
          <CCard className="tab-card">
            <Form
              innerRef={formRef}
              href="/admin/products_spec_attrs"
              table="products_spec_attrs"
              griduse
              {...methods}
              remainderChange={remainderChange}
              setFormSelected={setFormSelected}
            >
              <input type="hidden" name="id" ref={register()} defaultValue={products_spec_attrs ? products_spec_attrs.id : undefined} />
              <CRow className="mt-2">
                <CCol md="6" sm="12">
                  <CFormGroup>
                  ...
                  <CFormGroup>
                    ...
                    <CInputGroup className={errors.name && 'is-invalid'}>
                      <CInput
                        ...
                        innerRef={register({ required: true })}
                        defaultValue={products_spec_attrs ? products_spec_attrs.name : undefined}
                      />
                      <CInputGroupAppend>
                ...
                <CCol md="6" sm="12">
                  <CFormGroup>
                    <CLabel>{t('columns-products-spec-extraname')}</CLabel>
                    <CInputGroup className={errors.extra_name && 'is-invalid'}>
                      <CInput
                        ...
                        onChange={remainderChange}
                        innerRef={register()}
                        defaultValue={products_spec_attrs ? products_spec_attrs.extra_name : undefined}
                      />
                      <CInputGroupAppend>
                        <CInputGroupText className="bg-light text-muted">{remaining.name ? remaining.name : 0}/{maxLength.name}</CInputGroupText>
                      </CInputGroupAppend>
...
export default connect(mapStateToProps)(ProductsSpecAttrsForm);