...
const Form = (props) => {
  ...
  const MAIN_TABLE = location.pathname.replace(`/${SYS_LANG}`, '').replace(/^\/admin\//, '').replace(/\/\d+$/, '').replace(/\/add$/, '');
  const onSubmit = (data) => {
    let href = props.href;
  ...
  const formReset = (e) => {
    ...
    if (cFormElement && typeof cFormElement.elements.id === 'undefined') {
      ...
      if (props.classRelation) {
        ...
        //console.log([parent, parent_id_name]);
        props.setReactSelectOptions((reactSelectOptions) => {
          var values = [];
          var defaultValues = reactSelectOptions.defaultvalues ? reactSelectOptions.defaultvalues : [];
          if (reactSelectOptions.defaultvalues) {
            reactSelectOptions.defaultvalues[parent].forEach((item) => {
              values.push(item.value);
            });
          }

          if (reactSelectOptions.switchDefaultValues) {
            reactSelectOptions.switchValues[parent] = Object.assign({}, reactSelectOptions.switchDefaultValues[parent]);
          }

          if (cFormElement.elements[parent_id_name]) {
  ...
  const buildForm = (url, tableCheck, row) => {
    ...
    if (typeof row == 'object' && typeof row.options == 'object') {
      let _OPTIONS_ = row.options;
      let _VALUES_ = row.values;

      let defaultvalues = {};
      if (typeof row.defaultvalues != 'undefined') {
      ...
      if (typeof props.setReactSelectOptions == 'function') {
        ...
        if (typeof row.switch_values != 'undefined') {
          switch_values = row.switch_values;
          switch_default_values = Object.assign({}, row.switch_default_values);
        }
    ...
    const USE_TABLE = url.replace('/' + SYS_LANG + '/api/admin/', '').replace(/\?.*$/, '').replace(/\/\d+$/, '');
    let storeObj = {};
      ...
      fields.forEach((key) => {
        ...
        if (cFormElement && typeof cFormElement.elements[key] != 'undefined') {
          if (key.match(/^is_/)) {
            ...
            //for image field
          } else if (key.match(/(photo|path|image|video|audio)$/)) {
            if (!localStorage.getItem('copyId')) {
              let obj = {};
              obj[key] = {
                path: value
  ...
  const getUseRow = (url, loadingUse) => {
    ...
    if (accessoryTables.indexOf(tableCheck) > -1) {
      ...
      if (tableCheck == MAIN_TABLE && !localStorage.getItem('copyId')) {
        url = location.pathname.replace('admin', 'api/admin');
      } else {
  ...
  useEffect(() => {
    ...
    if (/^\d+$/.test(method_or_id)) {
      ...
      const url = ('/' + SYS_LANG + '/api' + href).replace(/\/+/, '');
      getUseRow(url);
    } else {
      if (localStorage.getItem('copyId')) {
        const copyId = localStorage.getItem('copyId');
        const url = apiBasePath.replace(/\/add$/, '/' + copyId);
        getUseRow(url, false);
      }
    }
