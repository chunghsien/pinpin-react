...
abstract class ECPay_Verification
{
    ...
    public function check_invoiceString($arExtend = [])
    {
        ...
        if (strlen($arExtend['CustomerPhone']) == 0 && strlen($arExtend['CustomerEmail']) == 0) {
        ...
        if ($arExtend['DelayDay'] < 0 || $arExtend['DelayDay'] > 15) {
        ...
        if ( ! array_key_exists('InvoiceItems', $arExtend)) {
            ...
            if ($arExtend['TaxType'] == ECPay_TaxType::Mix) {
                if (in_array(ECPay_TaxType::Dutiable, $tmpItemTaxType) && in_array(ECPay_TaxType::Free, $tmpItemTaxType)) {
                    // Do nothing
                } else {
                    $tmpItemTaxType = [];
                }
